// Every icon placed inside `src/assets/icons` will be
// embedded inside a sprite sheet included in every page.
// Use them with the provided shortcode: `{% icon "github" %}`.
// See `src/includes/default.njk` for an example.
// Thanks to https://github.com/maxboeck/eleventastic
  ...
  const compileSprite = async (args) =>
    new Promise((resolve, reject) => {
      spriter.compile(args, (error, result) =>
        error ? reject(error) : resolve(result.symbol.sprite)
      );
    });
